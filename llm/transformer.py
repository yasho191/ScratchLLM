...
class MultiHeadAttention(nn.Module):
    def __init__(self, embedding_size, num_heads, block_size, masked, dropout):
        ...
        self.attention_heads = nn.ModuleList([AttentionHead(embedding_size, hidden_dim, block_size, masked) for i in range(num_heads)])
...
class AttentionHead(nn.Module):
    def __init__(self, embedding_size, head_size, block_size, masked):
        super().__init__()  
        self.masked = masked
    ...
    def forward(self, x):
        ...
        if self.masked:
            QK = QK.masked_fill(self.tril[:time_step, :time_step] == 0, float('-inf'))
...
class Encoder(nn.Module):
    def __init__(self, vocab_size, embedding_size, num_layers, block_size, num_heads, dropout, classifier_hidden_dim, classifier_dim):
        ...
        self.positional_embedding = nn.Embedding(block_size, embedding_size)
    ...
    def forward(self, x):
        ...
        pos_embd = self.positional_embedding(torch.arange(time_steps, device=device))
        
        x_in = embd + pos_embd
        # Send Dummy Input for 1st Block -> Attention Maps
        block_out, attention_maps = self.blocks((x_in, torch.ones(1, 2, 2).to(device)))
